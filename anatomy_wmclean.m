function anatomy_wmclean(subj, cfg)
...
system(['cp ',fullfile(anatomy_preproc_dir,'wm.mgz'),' ',fullfile(anatomy_preproc_dir,'wm_old.mgz')]);

wmnew = wm;
wmnew.anatomy = volumeedit(wm.anatomy, 'background', mri.anatomy);

cfg = [];
cfg.filename  = fullfile(anatomy_preproc_dir,'wm.mgz');
cfg.filetype  = 'mgz';
